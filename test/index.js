...
describe('API ENDPOINT TESTING', () => {
    ...
    it('GET Detail Page', (done) => {
        chai.request(app).get('/api/v1/member/detail-page/5e96cbe292b97300fc902230').end((err, res) => {
            expect(err).to.be.null
            expect(res).to.have.status(200)
            expect(res).to.be.an('object')/
    ...
    it('POST Booking Page', (done) => {
        ...
        chai.request(app).post('/api/v1/member/booking-page')
            ...
            expect(res.body.message).to.be.equal('Success booking')
            expect(res.body).to.be.property('booking')
            expect(res.body.booking).to.be.all.keys(
                'payments',
                '_id',
                'invoice',
                'bookingStartDate',
